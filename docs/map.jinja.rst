...
We will see a quick setup to configure the ``fail2ban`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``fail2ban`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``fail2ban`` formulas under ``/srv/salt/fail2ban/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/fail2ban/parameters/dns:domain/
    ln -s dns:domain /srv/salt/fail2ban/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/fail2ban/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/fail2ban/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``fail2ban`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``fail2ban`` formulas under ``/srv/salt/fail2ban/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/fail2ban/parameters/roles
...
We create a configuration for the role ``fail2ban/server`` in ``/srv/salt/fail2ban/parameters/roles/fail2ban/server.yaml``:
...
We create another configuration for the role ``fail2ban/client`` in ``/srv/salt/fail2ban/parameters/roles/fail2ban/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles fail2ban/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles fail2ban/client
...
.. code-block:: console
    ...
    mv /srv/salt/fail2ban/parameters/dns:domain/  '/srv/salt/fail2ban/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-fail2ban-as-json:
      test.nop:
        - name: {{ fail2ban | json }}
...
Here is an example based on `template-formula/fail2ban/config/file.sls`_:
...
.. code-block:: sls
    ...
    fail2ban-config-file-file-managed:
      file.managed:
        ...
        - mode: 644
        - user: root
        - group: {{ fail2ban.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
          - sls: {{ sls_package_install }}
        - context:
            fail2ban: {{ fail2ban | json }}

This ``sls`` file expose a ``fail2ban`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ fail2ban['winner'] }}
...
.. _template-formula/fail2ban/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/fail2ban/config/file.sls
